...
<head>
    ...
    @if ($restaurant)
        <title>{{ $restaurant->restaurant_name }} Header</title>
    @else
        ...
        <header class="bg-white shadow-sm">
            <div class="container mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <nav class="hidden md:flex space-x-6">
                    <a href="{{ route('restaurant.show', ['id' => $restaurant->id ?? 1]) }}"
                        class="text-gray-600 hover:text-gray-900">Home</a>
                    <a href="{{ route('reservation', ['id' => $restaurant->id ?? 1]) }}"
                        class="text-gray-600 hover:text-gray-900">Reservation</a>
                    <a href="{{ route('restaurant', ['id' => $restaurant->id ?? 1]) }}"
                        class="text-gray-600 hover:text-gray-900">Menu</a>
                    <a href="{{ route('about', ['id' => $restaurant->id ?? 1]) }}"
                        class="text-gray-600 hover:text-gray-900">About Us</a>
                    <a href="{{ route('contact', ['id' => $restaurant->id ?? 1]) }}"
